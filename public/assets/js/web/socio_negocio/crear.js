...
function nuevoBanco() {
    ...
    var nuevo = `
            <tr id="tr_banco${id_banco}" class="clase_banco">
                ...
                <td>
                    <input type="radio" name="Predeterminado" class="Predeterminado" id="Predeterminado${id_banco}"  value="0" onchange="cambiarPredeterminado('${id_banco}')" />
                </td>
                <td></td>
                <td>
...
function verificarFormulario() {
    ...
    $.ajax({
        ...
        'data': { tipo: 'nuevo', ruc, docidentidad, razonsocial },
        'type': 'POST',
        'async': false,
        success: function (data) {
            var datos = JSON.parse(data);
...
$(function() {
    ...
    objSocioNegocio.grabar = function() {
        const form = $('form#form');
        let data = JSON.stringify(form.serializeObject());
        const method = ($('#_method').length > 0) ? $('#_method').val() : 'POST';
        $.ajax({
            ...
            if (resp.status == 422) {
                objMaster.printErrors(resp.errors);
            } else {
...
$(document).ready(function() {
    ...
    $('#btnGrabar').click(objSocioNegocio.grabar);
