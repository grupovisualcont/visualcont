...
function mostrarTipoDebeHaber() {
    ...
    $(".TipoDebeHaber").each(function (i) {
        ...
        $('#TipoDebeHaber' + i).attr('checked', false);
        ...
        if (!$('#TipoDebeHaber' + i).is(':disabled') && i == plan_contable_TipoDebeHaber) {
            $('#TipoDebeHaber' + plan_contable_TipoDebeHaber).attr('checked', true);
        }
    });
}
...
function mostrarTablaAmarres() {
    if ($('#Amarres').is(':checked')) {
        ...
        $.ajax({
            'url': BASE_URL + 'app/mantenience/accounting_plan/consulta_cuenta',
            'data': { CodCuenta, CuentaPadre, tipo: 'verificar_ultimo_hijo', subtipo: 'editar' },
            'type': 'POST',
            'async': false,
            success: function (data) {
...
function verificarFormulario() {
    ...
    $.ajax({
        'url': BASE_URL + 'app/mantenience/accounting_plan/consulta_cuenta',
        'data': { CodCuenta, NotCodCuenta: plan_contable_CodCuenta, DescCuenta, NotDescCuenta: plan_contable_DescCuenta, tipo: 'verificar_cuenta_hijo_editar', },
        'type': 'POST',
        'async': false,
        success: function (data) {
            var datos = JSON.parse(data);
