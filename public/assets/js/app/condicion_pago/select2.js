...
$(function () {
    ...
    s2CondicionPago = {
        ...
        init: function (objDOM, dropdownParent, width) {
            objDOM = (typeof objDOM == "undefined") ? $('select#condicion_pago') : objDOM;
            ...
            var select2 = objDOM.select2({
                ajax: {
                    url: BASE_URL + 'app/mantenience/payment_condition/autocompletado',
                    dataType: 'json',
                    delay: 250,
                    data: function (params) {
                        return {
                            search: params.term,
                            parameters: s2CondicionPago.parametros,
                            acciones: s2CondicionPago.acciones
                        };
                    },
                ...
                templateResult: function (res) {
                    ...
                    s2CondicionPago.selected.id = 0;
                    s2CondicionPago.validate = 0;
                    if (typeof res.id !== "undefined") {
                        var text = res.text;
                        // La operacion tendra otro tipo de vista
                        if (res.id == 'C' && s2CondicionPago.btnCreate) {
                ...
                templateSelection: function (res) {
                    if (res.id !== "") {
                        ...
                        if (s2CondicionPago.selected.id == 0) {
                            // Solo cuando aun no es selccionado se marca.
                            if (s2CondicionPago.validate == 0) {
                                if (res.id == "C") {
