...
$(function () {
    ...
    s2Anexo = {
        ...
        init: function (objDOM, dropdownParent, width) {
            objDOM = (typeof objDOM == "undefined") ? $('select#anexo_id') : objDOM;
            ...
            var select2 = objDOM.select2({
                ajax: {
                    url: BASE_URL + 'app/mantenience/anexo/' + this.urlMethod,
                    dataType: 'json',
                    delay: 250,
                    data: function (params) {
                        return {
                            search: params.term,
                            parameters: s2Anexo.parametros,
                            acciones: s2Anexo.acciones
                        };
                    },
                ...
                templateResult: function (res) {
                    ...
                    s2Anexo.selected.id = 0;
                    s2Anexo.validate = 0;
                    if (typeof res.id !== "undefined") {
                        var text = res.text;
                        // La operacion tendra otro tipo de vista
                        if (res.id == 'C' && s2Anexo.btnCreate) {
                ...
                templateSelection: function (res) {
                    if (res.id !== "") {
                        ...
                        if (s2Anexo.selected.id == 0) {
                            // Solo cuando aun no es selccionado se marca.
                            if (s2Anexo.validate == 0) {
                                if (res.id == "C") {
        ...
        activeSelect2: function (position) {
            if (!document.getElementById('anexo_id[' + position + ']')) {
                alert('Error al activar el Pluging Select2');
            } else {
                var combo = $(document.getElementById('anexo_id[' + position + ']'));
