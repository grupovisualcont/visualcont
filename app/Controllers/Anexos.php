    ...
    public function autocompletado()
    {
        try {
            ...
            if (isset($post['App']) && !empty($post['App']) && $post['App'] == 'Ventas') {
                ...
                $operaciones = (new Anexo())->getAnexo($this->CodEmpresa, 0, 0, '', 'CodAnexo AS id, DescAnexo AS text', [], 'CodInterno >= 101 AND CodInterno <= 109' . $search, '');
                ...
                $financiamientos = (new Anexo())->getAnexo($this->CodEmpresa, 0, 0, '', 'CodAnexo AS id, DescAnexo AS text', [], 'CodInterno >= 201 AND CodInterno <= 206' . $search, '');

                $option_financiamento[] = array('id' => "0", 'text' => 'Financiamiento', 'disabled' => true, 'class' => 'background-readonly h5 text-black');

                $inversiones = (new Anexo())->getAnexo($this->CodEmpresa, 0, 0, '', 'CodAnexo AS id, DescAnexo AS text', [], 'CodInterno >= 301 AND CodInterno <= 308' . $search, '');
                ...
                $anexo = (new Anexo())->getAnexo($this->CodEmpresa, $post['IdAnexo'], $post['TipoAnexo'], $post['OtroDato'], 'CodAnexo AS id, DescAnexo AS text', [], 'DescAnexo = "' . $post['DescAnexo'] . '"', '');
            } else {
                $Value = $post['Value'] ?? 'CodAnexo';
